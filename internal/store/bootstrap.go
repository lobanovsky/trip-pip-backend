...
//
// Публичного эндпоинта регистрации нет: агентства подключаются вручную, а
// самую первую учётную запись нельзя создать через аутентифицированный API.
// Делать это через переменные окружения — значит получить идемпотентность и
