...
`GET /api/ping` — проба живости, её использует `HEALTHCHECK` образа. `GET /api/version` возвращает коммит, из которого собран бинарник; для локальных сборок это `dev`, для образов из CI — SHA коммита. Скрипт развёртывания сверяет это значение и откатывается, если на сервере остался старый образ.
...
```json
{"time":"2026-08-12T22:44:51+03:00","level":"INFO","msg":"request","method":"GET","path":"/api/version","status":200,"duration_ms":0,"bytes":17,"request_id":"965a5df73445059b498fb9de3b46c113"}
```
